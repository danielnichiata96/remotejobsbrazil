...
export async function GET() {
  ...
  const filtered = jobs.filter((j) =>
    (j.tags || []).some((t) => /node|backend|back-end|express|nestjs|typescript/i.test(String(t)))
  );
  const xml = renderAtom(filtered, "Node.js");
...
function renderAtom(jobs: Awaited<ReturnType<typeof readJobs>>, label: string) {
  ...
  const feedId = siteUrl(`/feed/${label.toLowerCase().replace(/\./g, '')}.xml`);
  const items = jobs
    .map(
      (j) => `
    <entry>
